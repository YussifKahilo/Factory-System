...
public class FactorySystem {
	...
	public void SuperVisorUtility(Scanner in, SuperVisor user, Storage storage) {
		boolean logOut = false;
		while (!logOut) {
	...
	public void SalesManUtility(Scanner in, SalesMan user, Financial financial, Storage storage) {
		boolean logOut = false;
		while (!logOut) {
	...
	public void WorkerUtility(Scanner in, Worker user) {
		boolean logOut = false;
		while (!logOut) {
			UserMenu.workerMenu();
			int choice = in.nextInt();
	...
	public void userUtility(Person user, Manager manager, ArrayList<Worker> workers, ArrayList<SuperVisor> superVisors,
		...
		if (user instanceof Manager) {
			managerUtility(in, manager, workers, superVisors, salesMen, financial, storage, factory);
		} else if (user instanceof Worker) {
			Worker newUser = null;
			for (int i = 0; i < workers.size(); i++) {
				if (user.getId().equals(workers.get(i).getId())) {
					newUser = workers.get(i);
				}
			}
			WorkerUtility(in, newUser);
		} else if (user instanceof SuperVisor) {
			SuperVisor newUser = null;
			for (int i = 0; i < superVisors.size(); i++) {
				if (user.getId().equals(superVisors.get(i).getId())) {
					newUser = superVisors.get(i);
				}
			}
			SuperVisorUtility(in, newUser, storage);
		} else if (user instanceof SalesMan) {
			SalesMan newUser = null;
			for (int i = 0; i < salesMen.size(); i++) {
				if (user.getId().equals(salesMen.get(i).getId())) {
					newUser = salesMen.get(i);
				}
			}
			SalesManUtility(in, newUser, financial, storage);
		}
