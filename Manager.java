...
public class Manager extends Person {
     ...
     *
     * @param employee a reference from employee class to access its methods and
     * attributes a
     * @param promptionType if the promotion type is a supervisor the employee
     * will be promoted to supervisor otherwise it will be promoted to sales
     * man.
     ...
     * number of stored goods. 2) shows number of manufactured goods during the
     * month. 3) shows the sold goods. 4) sets the number of the goods that will
     * be sold and their prices.
     ...
     */
    public void storageManagment(int choice, Storage storage, Target target, int numberOfWorkers, int numberOfSalesMen) {
        Scanner input = new Scanner(System.in);
        boolean isValid = false;
        while (!isValid) {
            if (choice == 1) {
                ...
                if (numberOfWorkers > Long.parseLong(numOfGoodsToBeSold)) {
                    ...
                    target.setTargetOfGoodsToBeManufacturedForEachWorker(Long.parseLong(numOfGoodsToBeSold) / numberOfWorkers);
                    isValid = true;
                } else {
                    System.out.println("You can not set a target less than the number of workers .. \"HINT:NUMBER OF WORKERS IS :" + numberOfWorkers + "\"");
                }
            } else if (choice == 5) {
                ...
                if (Integer.parseInt(numOfGoodsToBeSold) > numberOfSalesMen) {
                    ...
                    target.setTargetOfGoodsToBeSoldByEachSalesMan(Integer.parseInt(numOfGoodsToBeSold) / numberOfSalesMen);
                    ...
                    System.out.println("You can not set a target less than the number of sales men .. \"HINT:NUMBER OF WORKERS IS :" + numberOfSalesMen + "\"");
                }
            }
        }
    }
     ...
     * @param target a reference from target to set the target in the storage.
     */
     ...
     * @param factory a reference from the factory to the fire the employee
     * from.
     */
     ...
     * @param financial a reference from financial to access to the total money
     * in in.
     */
